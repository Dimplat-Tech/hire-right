...
const DATA_PATH = path.join(process.cwd(), 'src', 'data', 'newsroom.json');
...
export async function POST(req: Request) {
  try {
    ...
    console.error('admin/newsroom POST error', err);
...
export async function PUT(req: Request) {
  try {
    ...
    console.error('admin/newsroom PUT error', err);
    return NextResponse.json({ error: 'Invalid payload' }, { status: 400 });
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    console.error('admin/newsroom DELETE error', err);
